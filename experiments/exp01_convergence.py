"""Experiment 01 -- Bayesian estimator convergence (regenerates Table 8).

Also produces the confusion matrix at n = 10 (Figure 6) and reports, for each
...
def run(n_trials: int = N_TRIALS) -> dict:
    ...
    for tier in ALL_TIERS:
        ...
        for _ in range(n_trials):
            ...
            state = est.__class__(model=est.model, privacy=est.privacy)  # fresh estimator
    ...
    table(rows, ["Tier", *[f"n={n}" for n in MILESTONES], "first>=90%"],
          "Table 8 (regenerated) -- classification accuracy (%), CORPUS-DP mode")

    conf_norm = confusion / confusion.sum(axis=1, keepdims=True)
    print("\nFigure 6 (regenerated) -- confusion matrix at n=10 (row = true tier)")
